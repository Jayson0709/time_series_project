...
def test(data):
    start = time.time()
    PAA = paa.PiecewiseAggregateApproximation(30)
    paa_dataset, reduced_data = PAA.transform(data)
    end = time.time()
    print('execution time is {}'.format(end-start))
    euclidean_distance = graph_accuracy_based_on_ed(data, paa_dataset)
    dtw_distance, warping_path = graph_accuracy_based_on_dtw(data, paa_dataset)
