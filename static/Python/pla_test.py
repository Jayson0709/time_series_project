...
def test(segment_size, data):
    ...
    euclidean_distance = graph_accuracy_based_on_ed(data, pla_dataset)
    dtw_distance, warping_path = graph_accuracy_based_on_dtw(data, pla_dataset)
    print('Euclidean distance is {}'.format(euclidean_distance))
    print('Dynamic Time Warping distance is {}'.format(dtw_distance))


test(54, fifty_words_data[1][1:])
test(60, computers_data[1][1:])
test(64, earthquake_data[1][1:])
test(70, ECG_data[1][1:])
test(199, symbols_data[1][1:])
